...
public class PongGameSourceCode extends PApplet {
...
public void setup() { //procedure which only runs once at the beginning of the program (built in)
   //ensures that the canvas size is 1000x1000px
}

public void draw() { //procedure which runs continuously throughout the program (built in)
...
public void mousePressed() { //procedure which only runs if the user clicks with their mouse (built in)
  if (startScreen) { //selection statement which runs if the user is in the start screen
    ...
    if (mouseX > 200 && mouseX < 800 && mouseY > 116.67f && mouseY < 216.67f) { //if the user clicks the outputted button
    ...
    if (mouseX > 200 && mouseX < 800 && mouseY > 616.67f && mouseY < 717.67f) { //if the users clicks anywhere on the outputted button
...
public void keyPressed() { //procedure which only runs if the user presses a button on the keyboard (built in)
...
class Ball { //class which contains all procedures of the ball
  ...
  public boolean paddleCollision() { //procedure which checks if the ball is about to collide with the user paddle on the next iteration
  ...
  public boolean gameOverCheck() { //procedure which checks if the game is over
    return y >= height; //procedure returns true if any one of the balls fall off the bottom of the canvas
  }
}
  public void settings() {  size(1000, 1000); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "PongGameSourceCode" };
    if (passedArgs != null) {
